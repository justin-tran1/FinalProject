...
historical_track_data <- historical_track_data[-c(2), ]


#use this function after the first row insert 


historical_track_data <- InsertRow(historical_track_data, track_features, 
                                   RowNum = NULL)
...
#a separate function to process the .csv file for individual countries. The big
#change is that you can note the company name and have it added to the file
#name of of the written .csv 
...
read_data_country <- function(filename, filedate, country )     {
 ...
 country_chart_edited <- country_chart[-c(1),]
  ...
  names(country_chart_edited) <- c("Position", "Track", "Artist", "Streams", 
                                  "track_uri")
...
track_features_country <- read_data_country("spotify_charts/Brazil/regional-br-weekly-2018-12-21--2018-12-28.csv", 
                            "2018-12-30", "Brazil")
...
historical_track_data_country <- setNames(data.frame(matrix(ncol = 10, nrow = 0)), 
                                  c("date", "avg_danceability", "avg_energy", "avg_loudness",
                                    "avg_speechiness", "avg_acousticness", 
                                    "avg_instrumentalnes", "avg_liveness", "avg_valence",
                                    "avg_tempo"))
...
historical_track_data_country <- InsertRow(historical_track_data_country, 
                                   track_features_country, RowNum = 1)

#delete the extra row the above creates 

historical_track_data_country <- historical_track_data_country[-c(15), ]
...
write.csv(df, paste0("data_averages/", country, "_data", ".csv"))
...
write_country("Brazil",historical_track_data_country)
